...
public class TransactionDAO {
    ...
    public static Transaction getTransaction(int transactionId) {
        ResultSet rs = SQLSession.runSQLQuery("SELECT * FROM transaction WHERE id=?", transactionId);
        return fetchTransaction(rs);
    }

    public static List<Transaction> getTransactions() {
        List<Transaction> list = new ArrayList<>();
        ResultSet rs = SQLSession.runSQLQuery("SELECT * FROM transaction");
        try {
            while(rs.next()) {
    ...
    public static ArrayList<Transaction> saveTransactions(List<Transaction> list) {
        ...
        for(Transaction t : list) {
            SQLSession.runSQLUpdate("INSERT INTO transaction (payer, archiving_code, date, " +
